...
import { getParticleSize } from '@/helpers/getParticleSize';
import { particleConfigs } from '@/lib/portfolioParticleConfigs';
...
const PortfolioSection = ({
  ...
  return (
    ...
    >
      ...
      {/* Particles with staggered positions */}
      {particleConfigs.map(({ variant, position }, i) => (
        <div
          key={i}
          ref={(el) => {
        ...
        >
          <Particle
            variant={variant}
            size={getParticleSize({ i, isSmallScreen })}
            delay={0}
            draggable
            opacity
          />
